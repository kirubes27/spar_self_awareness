...
class DelegateGameFromCapabilities(BaseGameClass):
    def __init__(
        ...
        self.prompts = {
            ...
            # Human input prompts
            "human_sa_prompt": "Your answer: ",
            "human_sa_choice_with_delegate": "Your choice (answer succinctly or T=Teammate): ",
            "human_mc_answer_prompt": "Your answer (A, B, C, D): ",
            "human_mc_choice_with_delegate": "Your choice (A, B, C, D, or T=Teammate): ",

            # LLM instruction lines
            ...
            "llm_sa_sysprompt": "Respond ONLY with your answer or 'T'.\n",
            "llm_mc_sysprompt": "Respond ONLY with your answer or 'T'.\n",
        ...
        # Resolve setup explanation with final N values
        ...
        if self.decision_only:
            ...
            # MC = 1 token; SA or certain models get None
        ...
        self.get_llm_answer_static_args = {
            "keep_appending": False,  # we use fresh history per Q
            "message_history": [],
            "MAX_TOKENS": max_tokens_used,
            "temp": self.temperature,
            "accept_any": False  # we pass explicit options
        }
    ...
    def _create_simulated_phase1_history(self):
        ...
        prompt = (self.prompts["human_mc_answer_prompt"] if not self.is_short_answer else self.prompts["human_sa_prompt"])
    ...
    def _run_phase2(self, final_feedback="", message_history=None, single_phase=False):
        ...
        if self.is_short_answer:
            ...
            setup_text += self.prompts["mc_setup_prompt"]
        ...
        for i, question in enumerate(self.phase2_questions, start=1):
            ...
            if self.decision_only:
                ...
                if action == "Delegate":
                    ...
                    self._record_trial(
                        ...
                        team_answer=None,
                        team_correct=subj_corr,  # team result equals subject choice in this path
                        probs=None
                    )
            ...
            if self.is_human_player:
                print(setup_text + "\n" + feedback_text + "\n" + q_text)
                if self.is_short_answer:
                    subject_decision = self._get_subject_answer(["T"], self.prompts["human_sa_choice_with_delegate"])
                    if subject_decision is None:
                        return False
                else:
                    valid_inputs = list(question["options"].keys()) + ["T"]
                    subject_decision = self._get_subject_answer(valid_inputs, self.prompts["human_mc_choice_with_delegate"])
                    if subject_decision is None:
                        return False
                ...
                current_history = base_history.copy()
                if self.is_short_answer:
                    prompt_line = self.prompts["human_sa_choice_with_delegate"]
                    llm_prompt = setup_text + "\n" + feedback_text + "\n" + q_text + "\n" + prompt_line
                    ...
                    prompt_line = self.prompts["human_mc_choice_with_delegate"]
                    llm_prompt = setup_text + "\n" + feedback_text + "\n" + q_text + "\n" + prompt_line
    ...
    def _save_game_data(self, message_history=None):
        """Persist full game data and run parameters."""
        game_data = {
            "subject_id": self.subject_id,
            "dataset": self.dataset,
...
def real_main(SUBJECT_NAME, DATASET):
    IS_HUMAN = False
    DECISION_ONLY = False            # Set to True for digits-only choice

    # Game parameters
    N_TRIALS_PHASE1 = 50
    N_TRIALS_PHASE2 = 500
    TEAMMATE_ACCURACY_PHASE1 = 0.3
    TEAMMATE_ACCURACY_PHASE2 = 0.3
    TEMPERATURE = 0.0
    SEED = 3
    resume_from = None
    ...
    # Setup description (basic; Phase 1 history/summary will provide detail)
    initial_setup_explanation = """
Let's play a game! You'll answer questions and can also delegate to a teammate in Phase 2.
Your goal is to maximize the number of correct answers (yours or your teammate's).
""".strip()

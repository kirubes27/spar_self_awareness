    ...
    for layer in args.layers:
        ...
        try:
            ...
            if layer >= all_hidden.shape[1]:
                print(
                    f"Error: Layer {layer} out of bounds (max {all_hidden.shape[1]-1}). skipping."
                )
                continue
            ...
            stats = analyze_layer(
                layer, H_L, labels, n_splits=args.n_splits, seed=args.seed, device=model.device
            )
            ...
            print(
                f"N total: {stats['n_total']} (Answer={stats['n_answer']}, Pass={stats['n_pass']})"
            )
